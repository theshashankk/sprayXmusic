...
async def generate_cover(requested_by, title, views, duration, thumbnail):
    ...
    image2 = Image.open("etc/coffinofficial.png")
...
async def playlist(client, message):
    ...
    msg = "**Now Playing** in {} 👻 via CoffinXmusic".format(message.chat.title)
    msg += "\n- "+ now_playing
    msg += "\n- Req by "+by
    temp.pop(0)
    if temp:
...
async def p_cb(b, cb):
    ...
    if type_ == 'playlist':           
        ...
        now_playing = temp[0][0]
        by = temp[0][1].mention(style='md')
        msg = "**Now Playing** in {} via coffinXmusic".format(cb.message.chat.title)
        msg += "\n- "+ now_playing
        msg += "\n- Req by "+by
        temp.pop(0)
        if temp:
...
async def play(_, message: Message):
           ...
           for administrator in administrators:
                      if administrator == message.from_user.id:  
                          ...
                          try:
                              ...
                              await lel.edit(
                                  "<b>@CoffinXPlayer userbot joined your chat</b>",
                              )
                              ...
                              await lel.edit(
                                  ...
                                  "\n\nOr manually add @CoffinXPlayer to your Group and try again</b>",
                              )
                              pass
    try:
        chatdetails = await USER.get_chat(chid)
        ...
        await lel.edit(
            f"<i> @CoffinXPlayer Userbot not in this chat, Ask admin to send /play command for first time or add assistant manually</i>"
        )
        return     
    ...
    if message.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        photo="final.png",
        reply_markup=keyboard,
        caption="✯CoffinXmusic✯▶️ **Playing** here the song requested by {} 😜".format(
...
async def deezer(client: Client, message_: Message):
    ...
    try:
        ...
        await lel.edit(
            f"<i>Looks like @CoffinXPlayer Userbot not in this chat, Ask admin to send /play command for first time or add assistant manually</i>"
        )
        return                            
...
async def jiosaavn(client: Client, message_: Message):
           ...
           for administrator in administrators:
                      if administrator == message_.from_user.id:  
                          ...
                          try:
                              ...
                              await USER.send_message(message_.chat.id,"I joined this group for playing music in VC")
                              await lel.edit(
                                  "<b>@CoffinXPlayer helper userbot joined your chat</b>",
                              )
    ...
    try:
        ...
        await lel.edit(
            "<i> @CoffinXPlayer helper Userbot not in this chat, Ask admin to send /play command for first time or add assistant manually</i>"
        )
        return     
    requested_by = message_.from_user.first_name
    chat_id=message_.chat.id
